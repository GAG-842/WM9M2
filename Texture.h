...
public:
	...
	void load(DxCore* core, string filename) {
        ...
        if (channels == 3) {
            ...
            Texture::init(core, width, height, channels, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB, texelsWithAlpha);
            delete[] texelsWithAlpha;
        }
        else {
            // Initialize texture using width, height, channels, and texels
			Texture::init(core, width, height, channels, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB, texels);
        }
        sampler.init(core);
		sampler.bind(core);
